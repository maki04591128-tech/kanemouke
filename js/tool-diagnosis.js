(function () {
  ...
  var RELATED = {
    ...
    // 利回りから毎月の必要積立額を逆算する、積立複利タブと表裏一体の
    // 計算）が、積立を始める全ての家族構成に共通して関連性が高いため、
    // 家族の状況ごとの個別の関連ツールに加えて必ず1件（既存の関連ツール
    // がある場合は2件目として）提示する。積立複利タブのFAQ自身も
    // 「目標金額から逆算して毎月いくら積み立てればいいか知りたい場合は？」
    // という設問でこのタブへ誘導しており、両タブは名指しで補完しあう
    // 関係にある。
    "tsumitate|spouse-child": [
      ...
      },
      {
        ...
        title: "必要積立額シミュレーター（目標から逆算）",
        desc: "教育資金など目標金額が決まっている場合は、逆算して毎月の必要積立額を確認できます。"
      }
    ],
    ...
    "tsumitate|spouse": [
      ...
      },
      {
        href: "pages/nisa-hub.html?tool=hitsuyou",
        title: "必要積立額シミュレーター（目標から逆算）",
        desc: "老後資金など目標金額が決まっている場合は、逆算して毎月の必要積立額を確認できます。"
      }
    ],
    "tsumitate|single": [{
      href: "pages/nisa-hub.html?tool=hitsuyou",
      title: "必要積立額シミュレーター（目標から逆算）",
      desc: "目標金額が決まっている場合は、逆算して毎月の必要積立額を確認できます。"
    }],
    "tsumitate|skip": [{
      href: "pages/nisa-hub.html?tool=hitsuyou",
      title: "必要積立額シミュレーター（目標から逆算）",
